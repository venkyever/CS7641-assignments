...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', default=42, type=int, help='A random seed to set, if desired')
    ...
    datasets = []
    dataset1_details = {
        'data': loader.TwitterSpamData(verbose=verbose, seed=seed),
        'name': 'twitter_spam',
        'readable_name': 'Twitter Spam Users',
        'best_nn_params': {'NN__activation': ['relu'], 'NN__alpha': [1.0],
                           'NN__hidden_layer_sizes': [(50, 5)], 'NN__learning_rate_init': [0.05]}
    }

    dataset2_details = {
        'data': loader.SpeedDatingData(verbose=verbose, seed=seed),
        'name': 'speed_dating',
        'readable_name': 'Speed Dating',
        'best_nn_params': {'NN__activation': ['relu'], 'NN__alpha': [0.31622776601683794],
                           'NN__hidden_layer_sizes': [(149,)], 'NN__learning_rate_init': [0.002]}
    }
    if args.dataset1:
