...
class DataLoader(ABC):
    ...
    def dump_test_train_val(self, test_size=0.2, random_state=123):
        ...
        print('before')
        print(train_y)
        test_y = pd.DataFrame(np.where(test_y == -1, -1, 1))
        train_y = pd.DataFrame(np.where(train_y == -1, -1, 1))
        validate_y = pd.DataFrame(np.where(validate_y == -1, -1, 1))
        print('after')
        print(train_y)
...
# class SpeedDatingData(DataLoader):
#     def __init__(self, path='data/speed_dating_df.csv', verbose=False, seed=1):
#         super().__init__(path, verbose, seed)
#
#     def _load_data(self):
#         print(self._path)
...
class TwitterSpamData(DataLoader):

    def __init__(self, path='data/twitter_df.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
